function TaskMtnAssignmentManager() {
	...
	let maintenanceTbody = maintenance.map((ele) => {
		...
		return (<tr className="hover" key={ele.mid}>
			<td>{ele.mid}</td>
			<td>{ele.mcategory}</td>
			<td>{ele.pid ? (<img className="inline-block object-scale-down w-12 mr-2" src={"http://localhost:8080/tofu/employee/task/getProductPhoto/" + ele.pid.productId} />) : null } {ele.pid ? getTheProductName(ele.pid) : ''}</td>
